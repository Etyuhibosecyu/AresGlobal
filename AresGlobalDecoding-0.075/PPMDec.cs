...
public class PPMDec
{
	...
	protected virtual void Initialize()
	{
		...
		excludingSet = [];
		globalSet = [];
		newItemsSet = blockIndex == 2 ? new() : new(new Chain((int)inputBase).Convert(x => ((uint)x, 1)));
		maxDepth = 12;
		var comparer = blockIndex == 2 ? (G.IEqualityComparer<NList<uint>>)new NListEComparer<uint>() : new EComparer<NList<uint>>((x, y) => x.Equals(y), x => unchecked(x.Progression(17 * 23 + x.Length, (x, y) => x * 23 + y.GetHashCode())));
