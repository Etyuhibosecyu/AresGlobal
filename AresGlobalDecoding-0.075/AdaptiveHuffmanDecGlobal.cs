...
public class AdaptiveHuffmanDecGlobal
{
	...
	public AdaptiveHuffmanDecGlobal(GlobalDecoding decoding, ArithmeticDecoder ar, NList<byte> skipped, LZData lzData, int lz, int bwt, int counter)
	{
	...
	protected virtual void DecodeIteration()
	{
		...
		{
			result.Add([uniqueList[readIndex]]);
			lzLength++;
	...
	protected virtual int ReadFirst()
	{
		...
		{
			var actualIndex = ar.ReadEqual(fileBase);
