...
public class LempelZivDec(NList<ShortIntervalList> compressedList, bool lz, LZData lzData, int tn)
{
	public NList<ShortIntervalList> Decode()
	{
		...
		for (var i = 0; i < compressedList.Length; i++, Status[tn]++)
		{
			...
			{
				j = (int)lzData.Length.R;
				length = compressedList[i][j].Lower;
			}
			else
			{
				j = (int)(3 - lzData.Length.R);
				length = compressedList[i][j].Lower + lzData.Length.Threshold + 2 - lzData.Length.R;
			}
			if (lzData.UseSpiralLengths == 0)
			{
				...
				{
					j2 = (int)(j + lzData.Dist.R);
					dist = compressedList[i][j2].Lower;
				}
				else
				{
					j2 = (int)(j + 3 - lzData.Dist.R);
					dist = compressedList[i][j2].Lower + lzData.Dist.Threshold + 2 - lzData.Dist.R;
				}
			}
			...
			if (readSpiralLength)
			{
				if (lzData.SpiralLength.R == 0)
					...
					spiralLength = compressedList[i][j2 + (int)lzData.SpiralLength.R].Lower;
				else
					spiralLength = compressedList[i][j2 + 3 - (int)lzData.SpiralLength.R].Lower + lzData.SpiralLength.Threshold + 2 - lzData.SpiralLength.R;
			}
			else
				spiralLength = 0;
