...
{
	public static void WorkUpDoubleList(out List<byte> result, List<ShortIntervalList> input, int tn) => result = WorkUpDoubleList(input, tn);
	...
	public static void WriteCount(this ArithmeticEncoder ar, uint originalBase)
	{
		...
		ar.WritePart((uint)t, 1, 31);
		var t2 = (uint)1 << Max(t, 1);
		ar.WritePart(originalBase - ((t == 0) ? 0 : t2), 1, t2);
	}
}
...
public class Huffman
{
	...
	public List<ShortIntervalList> Encode()
	{
		...
		Status[tn]++;
		result[0].Add(new(0, HuffmanApplied, HuffmanApplied));
		List<Interval> c = new();
		...
		Current[tn] += ProgressBarStep;
		if (maxFrequency > input[startPos][0].Base * 2 || input[startPos][0].Base <= 256)
			...
			for (var i = 0; i < frequencyTable.Length; i++, Status[tn]++)
				c.Add(new(frequency[i] >= 1 ? (uint)frequency[i] - 1 : throw new InvalidOperationException(), (uint)maxFrequency));
		uniqueList.Dispose();
...
public class LempelZiv
{
	...
	public LempelZiv(List<ShortIntervalList> input, List<ShortIntervalList> result, int tn)
	{
		this.input = input;
		this.result = result;
		this.tn = tn;
	...
	public List<ShortIntervalList> Encode()
	{
		var huffmanIndex = input[0].IndexOf(new(0, HuffmanApplied, HuffmanApplied));
		var huffman = huffmanIndex != -1;
		var I = 3 + (huffman ? (int)input[0][huffmanIndex + 1].Base : 0) + (input[0].Length >= 1 && input[0][0] == new Interval(0, LengthsApplied, LengthsApplied) ? (int)input[0][1].Base : 0);
		if (input.Length <= I)
			return LempelZivDummy(input);
		if (CreateVar(input[0].IndexOf(new(0, LempelZivApplied, LempelZivApplied)), out var lzIndex) != -1 && !(huffmanIndex != -1 && lzIndex == huffmanIndex + 1) && !(CreateVar(input[0].IndexOf(new(0, BWTApplied, BWTApplied)), out var bwtIndex) != -1 && lzIndex == bwtIndex + 1))
			return input;
		return huffman || input[0].Length >= 1 && input[0][0] == new Interval(0, WordsApplied, WordsApplied) ? EncodeInts(I) : EncodeBytes(I);
	}

	private List<ShortIntervalList> EncodeInts(int I)
	{
		var spaces = input[0].Length >= 2 && input[0][1] == new Interval(0, SpacesApplied, SpacesApplied);
		var (preIndexCodes, spaceCodes) = input.PNBreak((x, index) => index < I - 2 ? (-1, (byte)255) : ((int)((x[0].Lower << 9) ^ x[0].Base), (byte)(spaces ? x[^1].Lower : 0)));
		var spaceCodesActive = spaceCodes.Any(x => x is not 255 and not 0);
		Current[tn] = 0;
		CurrentMaximum[tn] = ProgressBarStep * 2;
		var combined = preIndexCodes.Skip(I - 2).NCombine(preIndexCodes.Skip(I - 1), (x, y) => ((ulong)(uint)x << 32) | (uint)y);
		var indexCodesList = combined.PGroup(tn, new EComparer<ulong>((x, y) => x == y, x => (int)(x >> 32) ^ (int)x)).FilterInPlace(x => x.Group.Length >= 2);
		combined.Dispose();
		...
		spaceCodes.Dispose();
		return WriteLZ(input, I, repeatsInfo, useSpiralLengths);
		void FindMatchesRecursive(uint[] ic, int level)
		{
			if (level < maxLevel)
			{
				var nextCodes = ic.AsSpan(..(ic[^1] + I - 2 == preIndexCodes.Length - level - startKGlobal ? ^1 : ^0)).GroupIndexes(iIC => preIndexCodes[(int)iIC + startKGlobal]).FilterInPlace(x => x.Length >= 2).Convert(x => x.ToArray(index => ic[index]).Sort());
		...
		void FindMatches(uint[] ic, int startK)
		{
			for (var i = 1; i < ic.Length; i++, Status[tn]++)
			{
				...
				if (maxReached.Length != 0 && Lock(lockObj, () => CreateVar(maxReached.IndexOfNotLess(iIC), out var mr) >= 1 && maxReachedLengths[maxReached[mr - 1]] > iIC - maxReached[mr - 1]))
					continue;
				var matches = ic.AsSpan((Array.FindLastIndex(ic, i - 1, x => iIC - x >= LZDictionarySize) + 1)..i).Filter(jIC => iIC - jIC >= 2 && RedStarLinq.Equals(preIndexCodes.AsSpan(iIC, startK), preIndexCodes.AsSpan((int)jIC, startK)) && RedStarLinq.Equals(spaceCodes.AsSpan(iIC, startK), spaceCodes.AsSpan((int)jIC, startK)));
				...
				for (; k <= ub && matches.Length > 1; k++)
				{
					...
					matches.FilterInPlace(jIC => preIndexCodes[iIC + k] == preIndexCodes[(int)(jIC + k)] && spaceCodes[iIC + k] == spaceCodes[(int)(jIC + k)]);
				...
				if (matches.Length == 1)
				{
					...
					for (; k <= ub2 && preIndexCodes[iIC + k] == preIndexCodes[lastMatch + k] && spaceCodes[iIC + k] == spaceCodes[lastMatch + k]; k++) ;
				}
				if (input.AsSpan(iIC, k).Sum(x => x.Sum(y => Log(y.Base) - Log(y.Length))) < Log(21) + Log(LZDictionarySize) + Log(k))
					continue;
				var sl = (ushort)Clamp(k / (iIC - lastMatch) - 1, 0, ushort.MaxValue);
				UpdateRepeatsInfo(repeatsInfo, lockObj, iIC, 0, (ushort)Max(iIC - lastMatch - k, 0), (ushort)Min(k - 2, iIC - lastMatch - 2), sl, PrimitiveType.UShortType);
				if (sl > 0)
					useSpiralLengths = 1;
		...
		void FindMatches2(uint[] ic, int k)
		{
			for (var i = 1; i < ic.Length; i++, Status[tn]++)
			{
				...
				if (maxReached.Length != 0 && Lock(lockObj, () => CreateVar(maxReached.IndexOfNotLess(iIC), out var mr) >= 1 && maxReachedLengths[maxReached[mr - 1]] > iIC - maxReached[mr - 1]))
					continue;
				var jIC = (int)ic[i - 1] + I - 2;
				if (!(iIC - jIC is >= 2 and < LZDictionarySize && RedStarLinq.Equals(preIndexCodes.AsSpan(iIC, k), preIndexCodes.AsSpan(jIC, k)) && RedStarLinq.Equals(spaceCodes.AsSpan(iIC, k), spaceCodes.AsSpan(jIC, k))))
					continue;
				if (input.AsSpan(iIC, k).Sum(x => x.Sum(y => Log(y.Base) - Log(y.Length))) < Log(21) + Log(LZDictionarySize) + Log(k))
					continue;
				var sl = (ushort)Clamp(k / (iIC - jIC) - 1, 0, ushort.MaxValue);
				UpdateRepeatsInfo(repeatsInfo, lockObj, iIC, 0, (ushort)Max(iIC - jIC - k, 0), (ushort)Min(k - 2, iIC - jIC - 2), sl, PrimitiveType.UShortType);
				if (sl > 0)
					useSpiralLengths = 1;
			}
		}
	...
	private List<ShortIntervalList> EncodeBytes(int I)
	{
		var preIndexCodes = input.Skip(I - 2).PNConvert(x => (byte)x[0].Lower);
		...
		void FindMatches(uint[] ic, int startK)
		{
			for (var i = 1; i < ic.Length; i++, Status[tn]++)
			{
				...
				if (input.AsSpan(iIC + I - 2, k).Sum(x => x.Sum(y => Log(y.Base) - Log(y.Length))) < Log(21) + Log(LZDictionarySize) + Log(k))
					continue;
				var sl = (ushort)Clamp(k / (iIC - lastMatch) - 1, 0, ushort.MaxValue);
				UpdateRepeatsInfo(repeatsInfo, lockObj, iIC + I - 2, 0, (ushort)Max(iIC - lastMatch - k, 0), (ushort)Min(k - 2, iIC - lastMatch - 2), sl, PrimitiveType.UShortType);
				if (sl > 0)
					useSpiralLengths = 1;
				if (k > ub || sl == ushort.MaxValue)
					Lock(lockObj, () => (maxReached.Add(iIC), maxReachedLengths.TryAdd(iIC, k)));
		...
		void FindMatches2(uint[] ic, int k)
		{
			for (var i = 1; i < ic.Length; i++, Status[tn]++)
			{
				...
				if (input.AsSpan(iIC + I - 2, k).Sum(x => x.Sum(y => Log(y.Base) - Log(y.Length))) < Log(21) + Log(LZDictionarySize) + Log(k))
					continue;
				var sl = (ushort)Clamp(k / (iIC - jIC) - 1, 0, ushort.MaxValue);
				UpdateRepeatsInfo(repeatsInfo, lockObj, iIC + I - 2, 0, (ushort)Max(iIC - jIC - k, 0), (ushort)Min(k - 2, iIC - jIC - 2), sl, PrimitiveType.UShortType);
				if (sl > 0)
					useSpiralLengths = 1;
			}
		}
	...
	private List<ShortIntervalList> WriteLZ(List<ShortIntervalList> input, int I, Dictionary<uint, (ushort dist, ushort length, ushort spiralLength)> repeatsInfo, uint useSpiralLengths)
	{
		...
		WriteLZMatches(input, I, useSpiralLengths, starts[..boundIndex], dists, lengths, spiralLengths, maxDist, maxLength, maxSpiralLength, rDist, thresholdDist, rLength, thresholdLength, rSpiralLength, thresholdSpiralLength, elementsReplaced, PrimitiveType.UShortType);
		...
		Parallel.ForEach(brokenRepeatsInfo, x => WriteLZMatches(input, I, useSpiralLengths, x.Item1, x.Item2.Item1, x.Item2.Item2, x.Item2.Item3, maxDist, maxLength, maxSpiralLength, rDist, thresholdDist, rLength, thresholdLength, rSpiralLength, thresholdSpiralLength, elementsReplaced, PrimitiveType.UShortType, false));
		result.FilterInPlace((x, index) => index == 0 || !elementsReplaced[index]);
		elementsReplaced.Dispose();
		result[0].Add(new(0, LempelZivApplied, LempelZivApplied));
...
#if DEBUG
		if (!RedStarLinq.Equals(Decoding.DecodeLempelZiv(result.Skip(I - 2), true, rDist, thresholdDist, rLength, thresholdLength, useSpiralLengths, rSpiralLength, thresholdSpiralLength, tn), input.Skip(I - 2), (x, y) => RedStarLinq.Equals(x, y, (x, y) => x.Equals(y) || x.Base == GetBaseWithBuffer(y.Base) && x.Lower == y.Lower && x.Length == y.Length)))
			throw new Exception();
#endif
		if (c.Length > 8)
			result[0].Add(new(0, LempelZivSubdivided, LempelZivSubdivided));
		result.Insert(1 + (input[0].Length >= 1 && input[0][0] == new Interval(0, LengthsApplied, LengthsApplied) ? (int)input[0][1].Base : 0), c.SplitIntoEqual(8).Convert(x => new ShortIntervalList(x)));
		return result;
	}
	...
	private void WriteLZMatches<T>(List<ShortIntervalList> input, int I, uint useSpiralLengths, NList<uint> starts, NList<T> dists, NList<T> lengths, NList<T> spiralLengths, T maxDist, T maxLength, T maxSpiralLength, int rDist, uint thresholdDist, int rLength, uint thresholdLength, int rSpiralLength, uint thresholdSpiralLength, BitList elementsReplaced, PrimitiveType type, bool changeBase = true) where T : unmanaged
	{
		...
		for (var i = 0; i < starts.Length; i++, Status[tn]++)
		{
			...
			var maxDist2 = Min(ToUInt(maxDist, type), (uint)(iStart - iLength - I));
	...
	{
		result.Replace(input);
		result[0].Add(new(0, LempelZivDummyApplied, LempelZivDummyApplied));
		List<Interval> list = new() { new(0, 3) };
		list.WriteCount(0, 16);
		list.Add(new(0, 3));
		list.WriteCount(0, 16);
		list.Add(new(0, 2));
		result.Insert(1 + (input[0].Length >= 1 && input[0][0] == new Interval(0, LengthsApplied, LengthsApplied) ? (int)input[0][1].Base : 0), new ShortIntervalList(list));
		return result;
	}
}
...
public class RLE
{
	...
	public byte[] Encode()
	{
		...
		for (var i = 1; i < originalFile.Length; i++, Status[tn]++)
		{
			if (currentSerie == 1)
			{
				if (originalFile[i] == previous)
				{
					...
					{
						result.AddRange(new List<byte> { 127, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128), originalFile[i] });
						length = 0;
						currentSerie = 2;
				...
				{
					result.AddRange(length >= 128 ? new List<byte> { 127, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128), previous } : new List<byte> { (byte)(length - 1), previous });
					length = 1;
					currentSerie = 2;
				}
			}
			else
			{
				if (originalFile[i] == previous && !doNotReadSerie1)
				{
					if (length >= 2)
					{
						result.AddRange(length >= 128 ? new List<byte> { 255, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128) } : new List<byte> { (byte)(length + 127) });
				...
				else
				{
					...
					if (length >= ValuesIn2Bytes)
					{
						result.AddRange(new List<byte> { 255, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128) });
		...
		if (currentSerie == 1)
			result.AddRange(length >= 128 ? new List<byte> { 127, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128), originalFile[^1] } : new List<byte> { (byte)(length - 1), originalFile[^1] });
		else
		{
			if (length != 0)
			{
				result.AddRange(length >= 127 ? new List<byte> { 255, (byte)((length - 127) >> BitsPerByte), (byte)(length - 127) } : new List<byte> { (byte)(length + 128) });
	...
	public byte[] RLE3()
	{
		...
		for (var i = 3; i < originalFile.Length; i += 3, Status[tn]++)
		{
			if (currentSerie == 1)
			{
				if (originalFile[i] == previous[0] && originalFile[i + 1] == previous[1] && originalFile[i + 2] == previous[2])
				{
					...
					if (length >= ValuesIn2Bytes)
					{
						result.AddRange(new List<byte> { 127, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128), originalFile[i], originalFile[i + 1], originalFile[i + 2] });
						length = 0;
						currentSerie = 2;
						doNotReadSerie1 = true;
						if (i > originalFile.Length - 1)
				...
				{
					result.AddRange(length >= 128 ? new List<byte> { 127, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128), previous[0], previous[1], previous[2] } : new List<byte> { (byte)(length - 1), previous[0], previous[1], previous[2] });
					length = 1;
					currentSerie = 2;
				}
			}
		...
		if (currentSerie == 1)
			result.AddRange(length >= 128 ? new List<byte> { 127, (byte)((length - 128) >> BitsPerByte), (byte)(length - 128), originalFile[^3], originalFile[^2], originalFile[^1] } : new List<byte> { (byte)(length - 1), originalFile[^3], originalFile[^2], originalFile[^1] });
		else
		{
			if (length != 0)
			{
